    ...
    def start(self):
        try:
            ...
            if len(frontend_domains) == 0:
                ...
                return
            self.log('Tn.BabaL'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(65535)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('Kesalahan.. Silahkan Hubungi Pembuat !'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('Silahkan hubungi pembuat', color='[R1]')
        except TimeoutError:
